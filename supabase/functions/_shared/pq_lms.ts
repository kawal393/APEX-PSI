...
/**
 * Master seed for the institutional LMS identity. Falls back to a stable
 * project-derived value so seals never fail, but APEX_PSI_SIGNING_SEED is
 * the authoritative source.
 */
export function lmsMasterSeed(): string {
  const raw = Deno.env.get("APEX_PSI_SIGNING_SEED")
    || Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")
    || "APEX-PSI-LMS-FALLBACK";
  return toHex(enc.encode(raw).slice(0, 32).length === 32
    ? enc.encode(raw).slice(0, 32)
    : concat(enc.encode(raw), new Uint8Array(32)).slice(0, 32));
}

/** Map a monotonic signature counter to {epoch, leaf} (rotate every 32). */
export function lmsSlot(counter: number): { epoch: number; leaf: number } {
  const n = Math.max(0, Math.floor(counter));
  return { epoch: Math.floor(n / LMS_LEAVES), leaf: n % LMS_LEAVES };
}

/**
 * Sign `message` with the institutional LMS identity for a given counter.
 * Counter comes from the ledger (number of PQ-sealed rows), so each one-time
 * leaf is used once and the key rotates automatically every 32 seals.
 */
export async function lmsSignInstitutional(
  message: Uint8Array,
  counter: number,
): Promise<LMSSignature> {
  const { epoch, leaf } = lmsSlot(counter);
  const { privateKey } = await deriveLMSKey(lmsMasterSeed(), epoch);
  return await lmsSign(message, privateKey, leaf);
}

/** Current institutional LMS public key (Merkle root) for a counter. */
export async function lmsInstitutionalPublicKey(counter = 0): Promise<string> {
  const { epoch } = lmsSlot(counter);
  const { publicKey } = await deriveLMSKey(lmsMasterSeed(), epoch);
  return publicKey;
}
