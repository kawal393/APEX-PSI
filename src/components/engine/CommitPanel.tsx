...
const CommitPanel = ({ onCommit, isProcessing, paused }: CommitPanelProps) => {
  ...
  return (
    <Card className="bg-engine-surface border-engine-border relative overflow-hidden">
      {/* Subtle animated border glow */}
      <div className="absolute inset-0 bg-gradient-to-br from-engine-approved/5 via-transparent to-transparent pointer-events-none" />
      
      <CardHeader className="pb-3">
        <CardTitle className="text-sm font-mono text-engine-muted uppercase tracking-widest flex items-center gap-2">
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-engine-approved opacity-75" />
            <span className="relative inline-flex rounded-full h-2 w-2 bg-engine-approved" />
          </span>
          Phase 1 — Commit
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4 relative">
        <div>
          <label className="text-xs font-mono text-engine-muted mb-1.5 block flex items-center gap-2">
            AI ACTION TO VERIFY
          ...
          <Textarea
            ...
            className="bg-engine-bg border-engine-border text-engine-text font-mono text-sm min-h-[100px] placeholder:text-engine-muted/50 focus-visible:ring-engine-approved/50 resize-none transition-all"
          />
          
          {/* Quick examples */}
          <div className="flex flex-wrap gap-1.5 mt-2">
            <span className="text-[10px] font-mono text-engine-muted">Quick test:</span>
            {EXAMPLE_ACTIONS.slice(0, 2).map((ex, idx) => (
              <button
                key={idx}
                onClick={() => loadExample(ex)}
                className="text-[10px] font-mono text-engine-muted/70 hover:text-engine-approved underline underline-offset-2 transition-colors cursor-pointer bg-transparent border-none"
              >
                Example {idx + 1}
              </button>
            ))}
        ...
        <div>
          ...
          <Select value={predicateId} onValueChange={setPredicateId}>
            ...
            <SelectContent className="bg-engine-surface border-engine-border max-h-[300px]">
              {PREDICATES.map((p) => (
                <SelectItem
                  key={p.id}
                  value={p.id}
                  className="font-mono text-xs text-engine-text focus:bg-engine-border focus:text-engine-text"
                >
                  <span className={`inline-block w-2 h-2 rounded-full mr-2 ${
                    p.riskLevel === 'UNACCEPTABLE' ? 'bg-engine-blocked' :
          ...
          <AnimatePresence mode="wait">
            {selectedPredicate && (
              <motion.div
                ...
                className="mt-2 p-3 rounded bg-engine-bg border border-engine-border overflow-hidden"
              >
                <div className="flex items-center gap-2 mb-1.5">
                  {selectedPredicate.riskLevel === 'UNACCEPTABLE' ? (
                    <AlertTriangle className="h-3.5 w-3.5 text-engine-blocked" />
                  ) : (
                    <Lock className="h-3.5 w-3.5 text-engine-muted" />
                  )}
                  <span className={`font-mono text-[10px] uppercase tracking-wider ${
                    selectedPredicate.riskLevel === 'UNACCEPTABLE' ? 'text-engine-blocked' :
                ...
                <p className="font-mono text-[11px] text-engine-muted leading-relaxed">
        ...
        <div className="flex items-center justify-between p-3 rounded bg-engine-bg border border-engine-border">
          ...
          <Switch
            ...
            className="data-[state=checked]:bg-engine-highlight"
          />
        </div>
        ...
        >
          <Button
            ...
            className={`w-full font-mono font-bold tracking-wider gap-2 transition-all duration-300 ${
              paused 
                ? 'bg-engine-blocked/20 border-engine-blocked/40 text-engine-blocked'
                : 'bg-engine-bg border border-engine-approved/40 text-engine-approved hover:bg-engine-approved/10 hover:shadow-engine-approved'
            } disabled:opacity-40`}
          >
            {paused ? (
              'SYSTEM PAUSED'
              ...
              <span className="flex items-center gap-2">
                <span className="h-4 w-4 border-2 border-engine-approved border-t-transparent rounded-full animate-spin" />
                HASHING...
              </span>
            ) : (
              <>
