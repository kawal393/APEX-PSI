...
import type { MerkleTreeState } from "@/lib/engine-core";
...
const MerkleVisualizer = ({ treeState }: MerkleVisualizerProps) => {
      ...
      <Card className="bg-engine-surface border-engine-border">
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center py-12 space-y-4">
            <div className="w-20 h-20 rounded-full border-2 border-dashed border-engine-border flex items-center justify-center">
              <GitBranch className="h-8 w-8 text-engine-muted/50" />
            </div>
            <p className="text-engine-muted font-mono text-sm text-center">
  ...
  return (
    <Card className="bg-engine-surface border-engine-border overflow-hidden">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-mono text-engine-muted uppercase tracking-widest flex items-center gap-2">
            <TreeDeciduous className="h-4 w-4 text-engine-approved" />
            Merkle Tree — Live State
          </CardTitle>
          <div className="flex items-center gap-3">
            <Badge className="bg-engine-bg border border-engine-border text-engine-muted font-mono text-xs gap-1">
              <Layers className="h-3 w-3" />
              {treeState.depth} layers
            </Badge>
            <Badge className="bg-engine-bg border border-engine-border text-engine-muted font-mono text-xs">
      ...
      <CardContent>
              ...
              >
                <div className="flex items-center gap-2 mb-1.5">
                  <span className={`text-[10px] font-mono w-12 shrink-0 ${
                    isRoot ? 'text-amber-400 font-bold' : isLeaf ? 'text-engine-approved' : 'text-engine-muted'
                  }`}>{label}</span>
                  <div className="h-px flex-1 bg-engine-border" />
                  <span className="text-[9px] font-mono text-engine-muted/50">{layer.length} node{layer.length > 1 ? 's' : ''}</span>
                </div>
                    ...
                    <motion.div
                      ...
                      className={`px-2 py-1 rounded border font-mono text-[10px] break-all max-w-[200px] truncate cursor-default transition-all hover:scale-105 ${
                        isRoot
                          ...
                          : isLeaf
                          ? 'border-engine-approved/30 bg-engine-approved/5 text-engine-approved/80'
                          : 'border-engine-border bg-engine-bg text-engine-muted'
                      }`}
                      title={hash}
                    >
                      {hash.substring(0, 16)}…
        ...
        {/* Stats */}
        <div className="mt-4 pt-3 border-t border-engine-border flex gap-4 text-xs font-mono text-engine-muted">
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-engine-approved/50" />
